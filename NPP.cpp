...
void NPP(vector<Process>&arr) {
  ...
  for(int i=0;i<n;i++){
    ...
    sort(arr.begin(),arr.end(),compareArrThenPriority);
    for(int i=0;i<n;i++)arr[i].indx=i;
    // ready queue which sorts Processes according to priority
    priority_queue<Process,vector<Process>,compPriority>pq;
    while(completed<n){
        // adding process with arrival time less than or equal current time to ready queue
       while(curindx<n&&arr[curindx].arrivaltime<=curtime)pq.push(arr[curindx++]);
       if(!pq.empty()){
